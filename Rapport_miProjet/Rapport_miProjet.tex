...
Ce projet a pour objectif la réalisation d'une application capable de générer un emploi du temps, et ainsi de faciliter le travail de M. Maeso. En effet, chaque année, il doit créer manuellement l'intégralité des emplois du temps de l'école, en tenant compte de toutes les contraintes de disponibilités des professeurs, des salles, des élèves, etc. L'application pourra lui faire gagner un temps non-négligeable dans son travail.\\
...
\newpage
\section{Les limites}
Le problème étant très complexe, ("np-complet"), nous ne pourrons pas faire un algorithme optimisé. En effet, notre projet s'inscrit dans le cadre de l'heuristique. L'heuristique, est utilisée en Mathématiques Discrètes afin de trouver une solution réalisable, mais pas nécessairement optimale.
De plus, au regard du nombre de contraintes associées au problème, nous devrons commencer par une instance simple du problème, en écartant un maximum de contraintes pour nous faciliter la tâche. Ainsi, au fur et à mesure que le projet avancera, on le complexifiera en incluant de nouvelles contraintes, jusqu'à arriver aux conditions réelles. Mais, le temps imparti étant limité, nous ne savons pas s'il nous sera possible d'inclure tous les facteurs liés à la planification dans notre logiciel.
...
Les paramètres sont inscrits dans des fichiers externes, et toutes les manipulations citées ci-dessus seront faites dans ces fichiers. Ainsi, l'utilisateur peut modifier le fichier depuis le programme ou directement en ouvrant le fichier.

Une fois tous les paramètres entrés, il peut génèrer les emplois du temps. Ces emplois du temps sont également mis dans des fichiers externes.

\subsection{Pré-traitement}
Après que les données aient été saisies par l'utilisateur, nous effectuons un pré-traitement pour éviter des calculs inutiles qui n'aboutiront pas.
Pour chaque matière, nous faisons la somme des cours qui doivent être donnés, toutes classes confondues, sur une même semaine. Cette somme doit nécessairement être inférieure ou égale au nombre de créneaux des professeurs qualifiés.
...
Ensuite, nous prenons le premier professeur de la liste (c'est à dire celui ayant le moins de disponibilités). 
Nous sélectionnons une matière, parmi celles qu'il enseigne, et nous comparons ses disponibilités à celles de la première classe ayant cette matière. Dès que nous trouvons un créneau disponible pour la classe et le professeur, on leur associe la matière et on met à jour leurs mots binaires afin que ce créneau ne soit plus utilisé.
Nous répétons cette manoeuvre jusqu'à ce que toutes les matières aient été placées.
