...
\begin{algorithmic}
\STATE $meilleurEDT \leftarrow 3000$
\REPEAT
\FORALL{$Promo$}
...
\STATE $planningOk \leftarrow $repartitionCoursPromotions($Promo, programmeSemestre$)
\ENDFOR
\IF {$planningOk$}
\IF {$nbCoursNonPlaces < meilleurEDT$}
\STATE $meilleurEDT \leftarrow nbCoursNonPlaces$
\STATE Ecriture des emploi du temps dans les fichiers
\ENDIF
\ENDIF
