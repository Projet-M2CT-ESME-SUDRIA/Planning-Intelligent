...
\IF {semaineDebut de $cours \leq semaine  \AND $ semaineFin de $cours > semaine$}
...
\IF {$prof$ est disponible à $semaine, creneau \AND prof$ est disponible $semaine, créneau +1 \AND classe$ est disponible à $semaine, créneau \AND classe$ est disponible $semaine, creneau +1$}
\STATE planification $cours$ avec $prof$ sur $semaine$ et $creneau$
\STATE planification $cours$ avec $prof$ sur $semaine$ et $creneau +1$
\ENDIF
\ELSE
\IF {$prof$ est disponible à $semaine, creneau \AND$ $prof$ est disponible $semaine, créneau +1$}
\STATE planification $cours$ avec $prof$ sur $semaine$ et $creneau$
\ENDIF
\ENDIF
\end{algorithmic}
